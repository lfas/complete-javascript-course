...
var UIController = (function() {
	...
	var formatNumber = function(num, type) {
		...
		// + or - before number, 2 decimal points, comma the thousands
		num = Math.abs(num);
		num = num.toFixed(2);
	...
	var nodeListForEach = function(list, callback) { /* O for vai executar o código definido na callback quando a função é chamada */
		for (var i = 0; i < list.length; i++) {
			callback(list[i], i); /* A posição e o valor são enviados para a callback */
		}
	};
...
var controller = (function(budgetCtrl, UICtrl) {
	...
	var ctrlAddItem = function() {
		...
		if (input.description !== "" && !isNaN(input.value) && input.value > 0) {
			...
			// 6. Calculate and updade percentages
			updatePercentages();
		}
	...
	var ctrlDeleteItem = function(event) {
		...
		if (itemID) {
			...
			// 4. Calculate and updade percentages
			updatePercentages();			
		}
	};
